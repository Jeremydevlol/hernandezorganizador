...
export default function Sidebar({
    ...
    return (
        <>
            <aside className="min-w-0 flex-1 flex flex-col bg-[var(--bg-dark)] border-r border-white/5 overflow-hidden">
                {/* Header */}
                <div className="px-4 py-3.5 border-b border-white/5 electron-drag">
                    <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center">
                            <Wheat className="w-4 h-4 text-emerald-400" />
                        </div>
                        <span className="font-medium text-sm text-zinc-100 tracking-tight">Cuaderno Agrícola</span>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto py-2">
                    <div className="flex items-center justify-between px-3 py-1.5">
                        ...
                        <div className="flex items-center gap-0.5">
                            <button
                                ...
                                className="w-7 h-7 flex items-center justify-center rounded-md text-zinc-500 hover:text-emerald-400 hover:bg-white/5 transition-colors"
                    ...
                    <div className="px-2">
                        <button
                            ...
                            className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md text-[11px] font-medium text-zinc-400 hover:text-zinc-200 hover:bg-white/5 transition-colors uppercase tracking-wider"
                        >
                        ...
                        {cuadernosExpanded && (
                            <div className="mt-1 space-y-0.5">
                                {loading ? (
                                    <div className="px-4 py-2 text-xs text-zinc-500">Cargando...</div>
                                ) : cuadernos.length === 0 ? (
                                    <div className="px-4 py-2 text-xs text-zinc-500">Sin cuadernos</div>
                                ) : (
                                    cuadernos.map((c) => (
                                        <div
                                            key={c.id}
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity text-zinc-500 hover:text-red-400 hover:bg-red-500/10"
                    ...
                    {activeCuaderno && activeCuaderno.parcelas?.length > 0 && (
                        <div className="px-2 mt-4">
                            <div className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[11px] font-medium text-zinc-400 uppercase tracking-wider">
                                <Wheat size={12} />
                                ...
                                {(() => {
                                    ...
                                    return Object.entries(grouped).sort((a, b) => b[1].ha - a[1].ha).map(([cultivo, info]) => (
                                        <button
                                            ...
                                            className="w-full flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors text-zinc-400 hover:bg-white/5 hover:text-zinc-200"
                                        >
                                            <div className="w-2 h-2 rounded-full bg-emerald-500/60 shrink-0" />
                                            <span className="flex-1 text-left truncate">{cultivo}</span>
                                            <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-white/5 text-zinc-500">
                                                {info.count}
                                            </span>
                    ...
                    <div className="px-4 py-3 border-t border-white/5 bg-black/20">
                        <div className="text-[11px] space-y-1.5">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-zinc-300">{activeCuaderno.año}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-zinc-500">Total hectáreas</span>
